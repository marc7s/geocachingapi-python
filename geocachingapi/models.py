...
class GeocachingSettings:
    ...
    def __init__(
        self,
        environment: GeocachingApiEnvironment = GeocachingApiEnvironment.Production,
        trackables: array(str) = [],
        caches: array(str) = [],
        nearby_caches_setting: NearbyCachesSetting = None,
    ) -> None:
        """Initialize settings"""
...
class GeocachingUser:
    ...
    def update_from_dict(self, data: Dict[str, Any]) -> None:
        ...
        self.favorite_points = try_get_from_dict(
            data, "favoritePoints", self.favorite_points
        )
        self.souvenir_count = try_get_from_dict(
            data, "souvenirCount", self.souvenir_count
        )
        self.awarded_favorite_points = try_get_from_dict(
            data, "awardedFavoritePoints", self.awarded_favorite_points
        )
        self.membership_level_id = try_get_from_dict(
            data, "membershipLevelId", self.membership_level_id
        )
...
class GeocachingTrackableLog:
    ...
    def __init__(self, *, data: Dict[str, Any]) -> GeocachingTrackableLog:
        ...
        if "owner" in data:
            ...
            self.owner = None
        self.log_type = try_get_from_dict(
            data["trackableLogType"], "name", self.log_type
        )
        self.logged_date = try_get_from_dict(data, "loggedDate", self.logged_date)
...
class GeocachingTrackable:
    ...
    def update_from_dict(self, data: Dict[str, Any]) -> None:
        ...
        self.tracking_number = try_get_from_dict(
            data, "trackingNumber", self.tracking_number
        )
        self.kilometers_traveled = try_get_from_dict(
            data, "kilometersTraveled", self.kilometers_traveled, float
        )
        self.miles_traveled = try_get_from_dict(
            data, "milesTraveled", self.miles_traveled, float
        )
        self.current_geocache_code = try_get_from_dict(
            data, "currectGeocacheCode", self.current_geocache_code
        )
        self.current_geocache_name = try_get_from_dict(
            data, "currentGeocacheName", self.current_geocache_name
        )
